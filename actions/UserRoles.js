...
export default function ActionsUserRoles(state = {
    roles: {},
    lastFetchTimestamp: 0,
}, action) {
    switch (action.type) {
        case actions.USER_ROLES_SET_ROLES_INFORMATION:
            return typeof action.payload !== 'undefined' && action.payload !== null && typeof action.payload.length !== 'undefined'
                ? {
                    ...state,
                    roles: {
                        ...state.roles,
...
export const fetchUserRoles = (forceFetch) => {
    ...
    return (dispatch, getState) => {
        ...
        if (forceFetch || lastFetchTimestamp < now - (1000 * 60 * 5)) {
            return API.get(API.USER_ROLE).then((response) => {
