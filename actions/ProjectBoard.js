...
export default function ActionsProjectBoard(state = {
    ...
    switch (action.type) {
        ...
        case actions.PROJECT_BOARD_INITIALIZE_PROJECT:
            ...
            return !_isNil(action.id)
                ? {
                    ...
                    allProjects: {
                        ...state.allProjects,
                        [action.id]: typeof state.allProjects[action.id] !== 'undefined' && state.allProjects[action.id]
        ...
        case actions.PROJECT_BOARD_CHANGE_CAMPAIGNS_WRITING_OR_MUSIC_TEAM_REQUEST:
            if (!_isNil(action.projectId) && !_isNil(action.campaignId) && !_isNil(action.requestType)) {
                ...
                return campaignIndex !== -1 ? update(state, {
                    allProjects: {
                        [action.projectId]: {
                            campaigns: {
                                [campaignIndex]: {
        ...
        case actions.PROJECT_BOARD_ADD_CAMPAIGN:
            return typeof action.id !== 'undefined' && typeof state.allProjects[action.id] !== 'undefined' ? update(state, {
                allProjects: {
                    [action.id]: {
                        campaigns: {
                            $push: [
                                _merge(initialCampaignState, _get(action, 'payload', {}))
                            ]
                        }
                    }
                }
        ...
        case actions.PROJECT_BOARD_ADD_OR_UPDATE_SPOT:
            if (
                ...
                if (campaignIndex !== -1) {
                    const spotIndex = typeof action.spotId !== 'undefined' && action.spotId
        ...
        case actions.PROJECT_BOARD_REMOVE_SPOT:
            if (typeof state.allProjects[action.id] !== 'undefined') {
                ...
                if (campaignIndex !== -1) {
                    ...
                    return spotIndex !== -1 ? update(state, {
                        allProjects: {
                            [action.id]: {
                                campaigns: {
                                    [campaignIndex]: {
        ...
        case actions.PROJECT_BOARD_REMOVE_VERSION:
            if (typeof state.allProjects[action.id] !== 'undefined') {
                ...
                if (campaignIndex !== -1) {
                    ...
                    if (spotIndex !== -1) {
                        const versionIndex = state.allProjects[action.id].campaigns[campaignIndex].spots[spotIndex]
                            .versions.findIndex((version) => {
                            return version.id === action.payload.versionId;
                        });

                        return versionIndex !== -1 ? update(state, {
                            allProjects: {
                                [action.id]: {
                                    campaigns: {
                                        [campaignIndex]: {
        ...
        case actions.PROJECT_BOARD_CHANGE_CAMPAIGNS_USER_ROLE:
            if (
                ...
                if (campaignIndex !== -1) {
                    const userIndex = state.allProjects[action.projectId].campaigns[campaignIndex].users.findIndex((user) => {
                    ...
                    dispatch({
                        ...
                        payload: {
                            ...
                            campaigns: response.campaign.map(c => ({
                                ...
                                id: c.campaignId,
                                name: c.campaignName,
                                firstPointOfContactId: typeof c.firstPointOfContactId !== 'undefined' && c.firstPointOfContactId
                                    ? c.firstPointOfContactId : null,
                                musicTeam: c.requestMusicTeam,
                                musicTeamNotes: c.musicTeamNotes ? c.musicTeamNotes : '',
                                writingTeam: c.requestWritingTeam,
                                writingTeamNotes: c.writingTeamNotes ? c.writingTeamNotes : '',
                                spots: c.spot.map(s => ({
                                    id: _toNumber(s.id),
                                    name: s.spotName,
                                    notes: s.notes,
                                ...
                                users: typeof c.user !== 'undefined' && typeof c.user.map !== 'undefined'
                                    ? c.user.map((user) => ({
                                        ...initialUserState,
                                        userId: user.userId,
                                        roleId: user.roleId,
...
export const fetchProjectHistory = (projectId) => {
    return (dispatch) => {
        return API.get(API.PROJECT + '/' + projectId).then((response) => {
...
export const updateProjectNotesText = (projectNotes, projectId) => {
    return (dispatch) => {
        if (typeof projectId !== 'undefined' && typeof projectNotes !== 'undefined') {
            ...
            return API.put(API.PROJECT + '/' + projectId, API.makePutData({
                notes: projectNotes
            })).then(() => {
                return Promise.resolve();
            }).catch((error) => {
...
export const createProjectCampaign = (projectId, campaignId, campaignName) => {
    return (dispatch) => {
        return API.post(API.ASSIGN_CAMPAIGN_TO_PROJECT, API.makePostData({
...
export const removeProjectCampaign = (projectId, campaignId) => {
    return (dispatch) => {
        return API.del(API.CAMPAIGN + '/' + campaignId + '/' + projectId).then((response) => {
            dispatch({
...
export const removeProjectSpot = (projectId, campaignId, spotId) => {
    return (dispatch) => {
        return API.del(API.SPOT + '/' + spotId).then((response) => {
            dispatch({
...
export const addProjectVersion = (projectId, campaignId, spotId, versionId, versionName) => {
    return (dispatch) => {
        if (
            ...
            return API.post(API.ASSIGN_VERSION_TO_SPOT, API.makePostData({
                spot_id: spotId,
...
export const removeProjectVersion = (projectId, campaignId, spotId, versionId) => {
    return (dispatch) => {
        return API.del(API.ASSIGN_VERSION_TO_SPOT + '/' + versionId + '/' + spotId).then((response) => {
            dispatch({
...
export const updateOrCreateSpot = (projectId, campaignId, spotId, spotDetails) => {
    return (dispatch) => {
        if (typeof projectId !== 'undefined' && typeof campaignId !== 'undefined' && typeof spotDetails !== 'undefined') {
            if (typeof spotDetails.name !== 'undefined' && spotDetails.name) {
                ...
                spotDetails = {
                    ...
                    notes: typeof spotDetails.notes !== 'undefined' && spotDetails.notes.trim() ? spotDetails.notes.trim() : '',
                    numberOfRevisions: typeof spotDetails.numberOfRevisions !== 'undefined' ? spotDetails.numberOfRevisions : null,
                    firstRevisionCost: typeof spotDetails.firstRevisionCost !== 'undefined' ? spotDetails.firstRevisionCost : null,
                    graphicsIncluded: typeof spotDetails.graphicsIncluded !== 'undefined' ? spotDetails.graphicsIncluded : false,
                };
                ...
                if (spotId !== null) {
                    return API.put(API.SPOT + '/' + spotId, API.makePutData(dataObject)).then((response) => {
                        dispatch({
                    ...
                    return API.post(API.SPOT, API.makePostData(dataObject)).then((response) => {
...
export const changeCampaignsCreativeExecutive = (projectId, campaignId, executiveId) => {
    return (dispatch) => {
        if (typeof projectId !== 'undefined' && typeof campaignId !== 'undefined' && typeof executiveId !== 'undefined') {
            return API.post(API.ASSIGN_CONTACT_TO_PROJECT_CAMPAIGN, API.makePostData({
                project_id: projectId,
                campaign_id: campaignId,
...
/**
 * Change writing or music team request data in project'scampaign
 *
 * @param {number} projectId - ID of the project
 * @param {number} campaignId - ID of the campaign
...
export const toggleCampaignsWritingOrMusicTeamRequest = (projectId, campaignId, type, toggle, notes) => {
    ...
    return (dispatch) => {
        if (typeof projectId !== 'undefined' && typeof campaignId !== 'undefined') {
            return API.put(API.PROJECT_CAMPAIGN + '/' + projectId + '/' + campaignId, API.makePutData({
...
export const addOrChangeRoleOfUserInProjectCampaign = (projectId, campaignId, userId, roleId) => {
    ...
    return (dispatch) => {
        if (projectId && campaignId && userId) {
            return API.post(API.PROJECT_CAMPAIGN_PEOPLE, API.makePostData({
                project_id: projectId,
                campaign_id: campaignId,
                user_id: userId,
                role_id: roleId,
...
export const removeUserFromProjectCampaign = (projectId, campaignId, userId) => {
    projectId = typeof projectId !== 'undefined' && projectId ? projectId : null;
    campaignId = typeof campaignId !== 'undefined' && campaignId ? campaignId : null;
    userId = typeof userId !== 'undefined' && userId ? userId : null;
    ...
    return (dispatch) => {
        if (projectId && campaignId && userId) {
            return API.del(API.PROJECT_CAMPAIGN_PEOPLE + '/' + projectId + '/' + campaignId + '/' + userId).then((response) => {
                dispatch({
