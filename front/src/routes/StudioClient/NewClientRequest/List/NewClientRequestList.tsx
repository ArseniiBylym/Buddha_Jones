...
class NewClientRequestList extends React.Component<AppState, {}> {
    ...
    private initNewClientRequestIsEdit = (): void => {
        if (this.props.store) {
            this.newClientRequestIsEdit = this.props.store.clients.newClientsRequestList.map(
    ...
    private isTableTitles(title: string): boolean {
        if (title === 'studio_name' || title === 'name' || title === 'email' || title === 'phone') {
            return true;
        } else {
            return false;
        }
    ...
    private getTableWithData(): JSX.Element {
        if (this.props.store) {
            let tableRowsArr: JSX.Element[] = this.props.store.clients.newClientsRequestList.map(
                (clientRequest: NewClientRequest, ind: number) => {
                    let tableCellsArr: JSX.Element[] = Object.keys(clientRequest)
                        .filter((key: string) => {
                    ...
                    return (
                        <React.Fragment key={`tablerow-${ind}`}>
                            <TableRow>
                                {tableCellsArr}
                                {this.newClientRequestIsEdit.length > 0 &&
                                    this.newClientRequestIsEdit[ind] &&
                                    !this.newClientRequestIsEdit[ind].isEdit && (
            ...
            return (
                ...
                >
                    {this.props.store.clients.newClientsRequestList &&
                    this.props.store.clients.newClientsRequestList.length > 0
                        ? tableRowsArr
