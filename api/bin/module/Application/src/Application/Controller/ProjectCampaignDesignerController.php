...
class ProjectCampaignDesignerController extends CustomAbstractActionController
{
    public function get($projectId)
    {
        ...
        $data = $this->_campaignRepo->getCampaignProjectPeople('designer', $projectId, $campaignId, $offset, $length, null, $this->_siteUrl . 'thumb/profile_image/');
        $totalCount = $this->_campaignRepo->getCampaignProjectPeopleCount('designer', $projectId, $campaignId);
            ...
            if($project && $campaign && $user) {
                ...
                if ($existingProjectToCampaign) {
                    ...
                    if (!$projectCampaignUser) {
                        ...
                        $response = array(
                            'status' => 0,
                            'message' => 'Designer already added to project campaign.'
                        );
                    }
                } else {
                    $response = array(
