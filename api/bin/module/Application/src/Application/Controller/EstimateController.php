...
class EstimateController extends CustomAbstractActionController
{
    ...
    public function get($id)
    {
        ...
        if ($data) {
            ...
            $data['staffs'] = $this->_estimateRepo->getEstimateStaff($id);
    ...
    public function create($data)
    {
        ...
        $staffs = (array)json_decode(trim(isset($data['staffs']) ? $data['staffs'] : ''), true);
...
//        $totalEstimate = 0;
        ...
        foreach ($staffs as $staff) {
            if (isset($staff['staff_id']) && (int)$staff['staff_id']) {
                $checkStaff = $this->_staffRepository->find($staff['staff_id']);

                if ($checkStaff) {
                    $tempstaffList = [
                        'staff_id' => (int)$staff['staff_id'],
                        'hourly_rate' => (float)$checkStaff->getRate(),
                        'estimated_regular' => isset($staff['estimated_regular']) ? $staff['estimated_regular'] : 0,
                        'estimated_overtime' => isset($staff['estimated_overtime']) ? $staff['estimated_overtime'] : 0,
                        'estimated_doubletime' => isset($staff['estimated_doubletime']) ? $staff['estimated_doubletime'] : 0,
                        'total_amount' => isset($staff['total_amount']) ? $staff['total_amount'] : null
                    ];
        ...
        if ($spotId && $multiplier && count($workerList)) {
            ...
            foreach ($staffList as $row) {
                $estimateStaff = new RediEstimateToStaff();
                $estimateStaff->setEstimateId($estimateId);
                $estimateStaff->setStaffId($row['staff_id']);
                $estimateStaff->setHourlyRate($row['hourly_rate']);
                $estimateStaff->setEstimatedRegular($row['estimated_regular']);
                $estimateStaff->setEstimatedOvertime($row['estimated_overtime']);
                $estimateStaff->setEstimatedDoubletime($row['estimated_doubletime']);
                $estimateStaff->setTotalAmount($row['total_amount']);

                $this->_em->persist($estimateStaff);
            }
            ...
            if ($responseData) {
                ...
                $responseData['staffs'] = $this->_estimateRepo->getEstimateStaff($estimate->getId());
...
//            $estimate->setCreatedAt(new \DateTime('now'));
            ...
            if(count($staffList)) {
                $deleteStaffList = $this->_estimateToStaffRepository->findBy(['estimateId' => $id]);

                foreach ($deleteStaffList as $row) {
                    $this->_em->remove($row);
                }
            ...
            if ($responseData) {
                ...
                $responseData['staffs'] = $this->_estimateRepo->getEstimateStaff($id);
                $responseData['submittedTo'] = $this->_estimateRepo->getSubmittedTo($responseData['submittedTo']);
                $responseData['temporaryStaff'] = $this->_estimateRepo->getEstimateTemporaryStaff($estimate->getId());
                $responseData['outsideCost'] = $this->_estimateRepo->getEstimateOutsideCost($estimate->getId());
                $responseData['history'] = $this->_estimateRepo->getHistoryByEstimateId($estimate->getId(), $imagePath);
