...
//        $dql .= " ORDER BY a.id ASC";
    ...
    public function getEstimateStaff($id)
    {
        $dql = "SELECT 
                    a.estimateId, 
                    a.staffId, s.name,
                    ...
                    a.estimatedRegular, a.estimatedOvertime, a.estimatedDoubletime,
                    a.totalAmount
                FROM \Application\Entity\RediEstimateToStaff a 
                INNER JOIN \Application\Entity\RediStaff s
                  WITH s.id=a.staffId
                WHERE a.estimateId=:id";
    ...
    public function updateEstimateHourAndTotal($estimateId)
    {
        ...
        if ($estimate) {
            ...
            $updateWorkerTotalDql = "UPDATE redi_estimate_to_worker w
                                ...
                                SET 
                                    w.total_amount = (((u.hourly_rate * 1.0 * w.estimated_regular_hour) + (u.hourly_rate * 1.5 * w.estimated_overtime_hour) + (u.hourly_rate * 2.0 * w.estimated_doubletime_hour)) * e.multiplier)
            ...
            // Update hour value on redi_estimate_to_staff
            if ($estimate['timeUnit'] == 'D') {
                $updateStaffHourDql = "UPDATE redi_estimate_to_staff ets
                                    LEFT JOIN
                                        redi_staff s ON s.id = ets.staff_id 
                                    SET 
                                        ets.estimated_regular_hour = ets.estimated_regular * IFNULL(s.min_hour, 0),
                                        ets.estimated_overtime_hour = ets.estimated_overtime * IFNULL(s.min_hour, 0),
                                        ets.estimated_doubletime_hour = ets.estimated_doubletime * IFNULL(s.min_hour, 0)
                                    WHERE
                                        ets.estimate_id = :estimate_id";
                ...
                $updateStaffHourDql = "UPDATE redi_estimate_to_staff ets
                                    LEFT JOIN
                                        redi_staff s ON s.id = ets.staff_id 
                                    SET 
                                        ets.estimated_regular_hour = ets.estimated_regular,
                                        ets.estimated_overtime_hour = ets.estimated_overtime,
                                        ets.estimated_doubletime_hour = ets.estimated_doubletime
                                    WHERE
                                        ets.estimate_id = :estimate_id";
            }

            $updateStaffHourQuery = $this->getEntityManager()->getConnection()->prepare($updateStaffHourDql);
            $updateStaffHourQuery->bindParam('estimate_id', $estimateId);
            $updateStaffHourQuery->execute();


            // Update total amount value on redi_estimate_to_worker
            $updateStaffTotalDql = "UPDATE redi_estimate_to_staff ets
                                        INNER JOIN
                                    redi_estimate e ON e.id = ets.estimate_id
                                        INNER JOIN
                                    redi_staff s ON ets.staff_id = s.id 
                                SET 
                                    ets.total_amount = (((s.rate * 1.0 * ets.estimated_regular_hour) + (s.rate * 1.5 * ets.estimated_overtime_hour) + (s.rate * 2.0 * ets.estimated_doubletime_hour)) * e.multiplier)
                                    where IFNULL(ets.total_amount, 0)=0
                                    AND ets.estimate_id=:estimate_id";

            $updateStaffTotalQuery = $this->getEntityManager()->getConnection()->prepare($updateStaffTotalDql);
            $updateStaffTotalQuery->bindParam('estimate_id', $estimateId);
            $updateStaffTotalQuery->execute();
